        ...
        if (restantes == 0) {
            ...
            await beginTournament(newUser).then(data => {
                ...
                if (data != "error") {
                    ...
                    if (data.mensaje) {
                        //ToastAndroid.show('No hay nadie esperando partida, cuando aparezca un contrincante se a�adira la partida a tu lista de partidas', ToastAndroid.SHORT);
            ...
            ToastAndroid.show('Elija ' + restantes + ' amigo m�s', ToastAndroid.SHORT);
        } else {
            ToastAndroid.show('Elija ' + restantes + ' amigos m�s', ToastAndroid.SHORT);
        }
    }
    render() {
        let { restantes } = this.state
